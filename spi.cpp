...
void sendSPIFrames(hid_device* handle, const std::vector<uint8_t>& commandFrames, std::vector<uint8_t>& responseFrames) {
    ...
    // Transfert SPI via SPISendReceive
...
std::vector<uint8_t> buildCommandFrames(uint8_t command, const std::vector<uint16_t>& data) {
    std::vector<uint8_t> frames(NUM_POTS * 2, 0x00);
    for (int i = 0; i < NUM_POTS; ++i) {
        frames[i * 2] = command | ((data[i] >> 8) & 0x0F); // Commande et MSB des données
